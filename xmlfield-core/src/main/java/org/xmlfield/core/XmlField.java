...
public class XmlField {
    ...
    public String nodeToXml(final XmlFieldNode<?> node) throws XmlFieldParsingException {
        return parser.nodeToXml(node);
    }

    public XmlFieldNode<?> xmlToNode(final InputStream xmlContent) throws XmlFieldParsingException {
        return parser.xmlToNode(xmlContent);
    }

    public XmlFieldNode<?> xmlToNode(final String xml) throws XmlFieldParsingException {
     ...
     */
    public <T> T[] bindArray(String xml, Class<T> type) throws XmlFieldException {

        XmlFieldNode<?> node = xmlToNode(xml);
        T[] resultArray = new XmlField().bindToArray(getElementNameWithSelector(getResourceXPath(type)), node, type);
        return resultArray;
    ...
    public <T> T[] nodeToArray(final XmlFieldNode<?> node, final Class<T> type, final String xpathSelector)
            throws XmlFieldXPathException {
