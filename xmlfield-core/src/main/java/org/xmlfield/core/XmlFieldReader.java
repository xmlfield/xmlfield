...
public class XmlFieldReader {
    ...
    public <T> T attach(final String resourceXPath, final Node node, final Class<T> type) {
        ...
        if (resourceXPath == null) {
            ...
            try {
                ...
                subNode = (Node) xpath.evaluate(resourceXPath, node, XPathConstants.NODE);
    ...
    public <T> T[] attachArray(final String resourceXPath, final Node node, final Class<T> type)
        ...
        final NodeList nodeList = (NodeList) xpath.evaluate(resourceXPath, node, XPathConstants.NODESET);
    ...
    public <T> T[] attachArray(final String resourceXPath, final Node node, final Class<T> type,
        ...
        final NodeList nodeList = (NodeList) xpath
                .evaluate(resourceXPath + xpathSelector, node, XPathConstants.NODESET);
    ...
    public Object[] attachExplicitArray(final String resourceXPath, final Node node,
        ...
        final NodeList nodeList = (NodeList) xpath.evaluate(resourceXPathGlobal, node, XPathConstants.NODESET);
        ...
        for (int i = 0; i < nodeCount; ++i) {
            ...
            if (explicitCollection.containsKey(subNode.getNodeName())) {
                final Object proxy = loadProxy(subNode, explicitCollection.get(subNode.getNodeName()));
                list.add(proxy);
            }
     ...
     *             when parsing xml failed
     */
    public <T> T attachReadOnly(String xml, Class<T> type) throws ParserConfigurationException, //
